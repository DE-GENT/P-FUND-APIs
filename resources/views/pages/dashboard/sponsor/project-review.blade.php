...
<head>
    ...
    <title>Sponsor Project Review | P-FUNDS</title>
    <!-- Font Awesome CDN -->
...
<body>
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('sponsor.project-review') }}" class="nav-item active"><i class="fa-solid fa-clock-rotate-left"></i> Project Review</a>
    ...
    <main class="main-content">
        ...
        <!-- Reviewed Projects Table -->
        <div class="table-container">
            <div class="table-header">
                <h2>Previously Viewed / Evaluated</h2>
                <p>Verify details of projects you have previously saved, liked, disliked, or viewed.</p>
            </div>
            <table id="reviewed-projects-table">
                <thead>
                    <tr>
                        <th>Username</th>
                        <th>Project Name</th>
                ...
                <tbody id="reviewed-projects-tbody">
                    <tr><td colspan="4" class="empty-state"><i class="fa-solid fa-spinner fa-spin"></i> Loading reviewed projects...</td></tr>
                </tbody>
            </table>
        </div>
    </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            // Load data
            await loadReviewedProjects();

            // Setup logout button
        });

        async function loadReviewedProjects() {
            try {
                // Fetch reviewed projects
                const response = await apiClient.get('/sponsor/projects/reviewed');
                const projects = Array.isArray(response) ? response : (response.projects || []);
                renderReviewedProjects(projects);
            } catch (error) {
                console.error("Failed to load reviewed projects:", error);
                ...
                const errorMsg = `<tr><td class="pf-dedup-4b996d" colspan="4"><i class="fa-solid fa-triangle-exclamation"></i> ${message}</td></tr>`;
                document.getElementById('reviewed-projects-tbody').innerHTML = errorMsg;
            }
        }

        function renderReviewedProjects(projects) {
            const tbody = document.getElementById('reviewed-projects-tbody');
            tbody.innerHTML = '';

            if (projects.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" class="empty-state"><i class="fa-regular fa-folder-open"></i><br>No reviewed projects found.</td></tr>';
                return;
            }
            ...
            projects.forEach(project => {
                ...
                // If there's an interaction type saved in database
                const interactionText = project.interaction_type ? `<span class="pf-dedup-000b29">(${project.interaction_type})</span>` : '';

                tr.innerHTML = `
                    <td class="pf-dedup-cc5621"><i class="fa-regular fa-user pf-dedup-ed18db"></i>${escapeHtml(creatorName)}</td>
                    <td class="pf-dedup-3acea4">${escapeHtml(project.title)}${interactionText}</td>
                    <td class="pf-dedup-5b795a">${escapeHtml(previewDesc)}</td>
                    <td>
                        <button class="action-btn" onclick="viewDetails(${project.id})" title="See More"><i class="fa-solid fa-eye"></i> Details</button>
                    </td>
                `;
        ...
        async function interactWithProject(projectId, type) {
            try {
                ...
                // No alert for silent tracking like viewed
            } catch (error) {
